...
{
    [Serializable]
    [ActionType("migrate_legacy_shop")]
    public class MigrateLegacyShop : GameAction
    {
        ...
        public override IAccountStateDelta Execute(IActionContext context)
        {
            ...
            Log.Debug("Start Migrate Legacy Shop");
            ...
            foreach (var group in groupBy)
            {
                ...
                Log.Debug($"Start Migrate Avatar({avatarAddress}). Target Count: {group.Count()}");
                ...
                Log.Debug($"Finish Migrate Avatar({avatarAddress})");
            }

            states = states.SetState(Addresses.Shop, shopState.Serialize());
            Log.Debug($"Finish Migrate Legacy Shop({shopState.Products.Count})");
